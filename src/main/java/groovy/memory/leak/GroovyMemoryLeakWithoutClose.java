...
public class GroovyMemoryLeakWithoutClose {

	private static final Logger logger = LoggerFactory.getLogger(GroovyMemoryLeakWithoutClose.class.getName());
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for(int i = 1; i > 0; i++) {
			executor.execute(() -> {
				try {
					// 10
					GroovyUtil.executeScriptWithoutClose(binding, script + "\"" + UUID.randomUUID() + "\");");
					GroovyUtil.executeScriptWithoutClose(binding, script + "\"" + UUID.randomUUID() + "\");");
					GroovyUtil.executeScriptWithoutClose(binding, script + "\"" + UUID.randomUUID() + "\");");
					GroovyUtil.executeScriptWithoutClose(binding, script + "\"" + UUID.randomUUID() + "\");");
					GroovyUtil.executeScriptWithoutClose(binding, script + "\"" + UUID.randomUUID() + "\");");
